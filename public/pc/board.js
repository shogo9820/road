window.boardManager = {
  ...
  playerColors: ["#f44336", "#2196f3", "#4caf50", "#ff9800", "#9c27b0", "#00bcd4", "#e91e63", "#795548"],
  ...
  init() {
    this.canvas = document.getElementById("board-canvas"); if (!this.canvas) return;
    this.ctx = this.canvas.getContext("2d"); this.canvas.width = 60 * this.cellSize; this.canvas.height = 56 * this.cellSize;
  ...
  draw(playersList, activeIdx) {
    if (!this.ctx || !this.canvas) this.init(); if (!this.ctx) return;
    ...
    MAP_SQUARES.forEach((sq, idx) => {
      ...
      this.ctx.fillStyle = fillColor; this.ctx.fillRect(px, py, pw, ph);
      this.ctx.strokeStyle = (fillColor === "#ffffff") ? "#444444" : "#ffffff"; this.ctx.lineWidth = 1.5; this.ctx.strokeRect(px, py, pw, ph);
      this.ctx.fillStyle = (fillColor === "#ffffff") ? "#333333" : "#ffffff"; this.ctx.font = "bold 11px sans-serif"; this.ctx.textAlign = "center"; this.ctx.textBaseline = "middle";
    ...
    if (playersList && Array.isArray(playersList)) {
      ...
      playersList.forEach((pl, idx) => {
        const currentPos = pl.position !== undefined ? pl.position : 0; const coords = this.getCoordinates(currentPos);
        ...
        let offsetX = 0; let offsetY = 0; if (offsetIdx > 0) { const angle = (offsetIdx * Math.PI * 2) / 4; offsetX = Math.cos(angle) * 10; offsetY = Math.sin(angle) * 10; }
        const pinX = coords.x + offsetX; const pinY = coords.y + offsetY; const pinColor = pl.color || this.playerColors[idx % this.playerColors.length];
        ...
        if (idx === activeIdx) { this.ctx.lineWidth = 2; this.ctx.strokeStyle = "#ffffff"; this.ctx.beginPath(); this.ctx.arc(pinX, pinY, 9, 0, Math.PI * 2); this.ctx.stroke(); }
      });
    }
  },
